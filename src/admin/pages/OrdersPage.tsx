...
// Orders is the active queue now — completed/cancelled orders moved to the
// History tab so staff aren't scanning through finished orders here.
...
export function OrdersPage() {
  ...
  const buildFilters = () => {
    ...
    // Status filter — Orders only ever shows the active queue. A specific
    // active status narrows it further; otherwise show every active status.
    if (statusFilter) {
  ...
  if (error) {
    return (
      <div className="space-y-6 p-6">
        <div>
          <h1 className="text-3xl font-display text-ink mb-2">{t('orders.title')}</h1>
          <p className="text-moss">{t('page.ordersDescription')}</p>
        </div>
        <Card>
          <CardBody>
